...
const calculateRPGStats = (scores: GogyoScore, nikkanGogyo: string) => {
...
export const RPGStatusRadar: React.FC<Props> = ({ scores, nikkanGogyo, nikkanKan }) => {
  const data = calculateRPGStats(scores, nikkanGogyo);
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
        ...
        <div className="w-full md:w-1/2 text-center md:text-left space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 mt-6">
            {data.map((stat, i) => (
              <div key={i} className="bg-slate-900/80 backdrop-blur-md rounded-xl p-3 border border-slate-700/50 shadow-inner">
